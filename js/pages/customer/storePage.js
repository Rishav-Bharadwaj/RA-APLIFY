...
function renderStorePage(container, params = {}) {
    ...
    const deliveryInfo = store.home_delivery_enabled
        ? `<span class="badge badge-delivery">${Icons.truck} Home Delivery • Within ${store.delivery_radius_km} km</span>`
    ...
    container.innerHTML = `
        ...
        <div class="page-content" style="padding-bottom:${DataStore.getCartCount() > 0 ? '140px' : '80px'}">
            ...
            <div class="store-detail-header">
                ...
                <p class="store-detail-category">${store.category} • ${store.location}</p>
                <div class="store-detail-meta">
                    <div class="store-meta-item">
                        ${Icons.star} <strong>${store.rating}</strong> (${store.review_count})
                    </div>
                    <div class="store-meta-item">
                        ${Icons.clock} ${store.open_time} - ${store.close_time}
                    </div>
                </div>
            ...
            <div style="padding:var(--space-4)">
                <p class="text-sm text-secondary">${store.description}</p>
            </div>
